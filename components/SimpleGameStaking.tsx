...
interface SimpleGameStakingProps {
  gameId: string;
  stakeAmount: number;
  onStakeSuccess?: (hash: string) => void;
}

export const SimpleGameStaking: React.FC<SimpleGameStakingProps> = ({
  gameId,
  stakeAmount,
  onStakeSuccess
}) => {
  const { stakeBNB, isLoading, error, success } = useSimpleStaking();
  const [hasStaked, setHasStaked] = useState(false);

  const handleStake = async () => {
    try {
      const result = await stakeBNB(stakeAmount, gameId);
      
      if (result.success) {
        setHasStaked(true);
        if (onStakeSuccess && result.hash) {
          onStakeSuccess(result.hash);
        }
      }
    } catch (error) {
...
export default SimpleGameStaking;
